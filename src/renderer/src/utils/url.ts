...
export function createUrl<P extends Record<string, string>, T extends Record<string, string>>(base: string, check: string): Url<P, T> {
  const baseUrl = base.split('/').filter(s => !!s)
  const indexOfSearch = check.indexOf('?')
...
export function toUrl<T extends Record<string, string>>(check: string): Url<{}, T> {
  const indexOfSearch = check.indexOf('?')
  const checkPath = check.substring(0, indexOfSearch == -1 ? undefined : indexOfSearch)
  const checkSearch: any = {}
  const indexOfHash = check.indexOf('#')
...
export interface Url<TParams extends Record<string, string>, TQuery extends Record<string, string>> {
  params: TParams,
  fullPath: string,
  path: string,
  query: TQuery,
